...
export default function DefenceCard({ title, description, url, imageUrl, source, date, summaryText }) {
  ...
  return (
    ...
    >
      {imageUrl ? (
        <img src={imageUrl} alt={title} style={{ width: '100%', height: '160px', objectFit: 'cover' }} />
      ) : (
        <div style={{ width: '100%', height: '160px', background: 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#4B5563' }}>
          No Image
        </div>
      )}
      <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', flex: 1 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: 'auto' }}>
          ...
          <SummaryButton text={`${title}. ${summaryText}`} variant="dark" />
        </div>
      </div>
    </div>
  );
